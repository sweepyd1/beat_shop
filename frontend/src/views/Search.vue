<template>
  <div class="search">
    ...
    <div class="active-filters" v-if="hasActiveFilters">
      <span class="chip" v-for="genre in selectedGenres" :key="genre.id" @click="removeGenre(genre)">
      ...
      <span class="chip" v-if="durationRange[0] !== 120 || durationRange[1] !== 360" @click="resetDuration">
        Длит.: {{ Math.round(durationRange[0]/60) }}-{{ Math.round(durationRange[1]/60) }} мин <i class="fas fa-times"></i>
      </span>
    ...
    <div class="search-content" :class="{ 'filters-open': showFilters }">
      <aside class="filters" v-if="showFilters">
        ...
        <div class="range">
          ...
          <div class="range-values">
            <span>{{ Math.round(durationRange[0]/60) }} мин</span> — <span>{{ Math.round(durationRange[1]/60) }} мин</span>
          </div>
        </div>
      ...
      <main class="results">
        ...
        <div v-else-if="tracks.length === 0" class="empty">Ничего не найдено</div>

        <div class="track-grid" :class="[viewMode]" v-else-if="viewMode === 'grid'">
          <TrackCard
        ...
        <div class="track-list" v-else>
          ...
          >
            ...
            <img :src="track.cover_url" alt="" class="track-cover" @error="handleImageError" />
...
/* Кастомные чекбоксы */
.filter-option { display: flex; align-items: center; margin-bottom: 0.5rem; position: relative; }
