...
const deleteUser = async (id) => {
  alert(
    ...
    '• Профиль автора (если пользователь является автором)\n' +
    '• Все треки этого автора вместе с обложками и аудиофайлами\n' +
    '• Все покупки треков (включая покупки, совершённые другими пользователями)\n' +
    '• Все контракты, связанные с этими покупками\n' +
    '• Все избранные (лайки) и подписки на этого автора\n' +
    '• Вся статистика прослушиваний и взаимодействий\n\n' +
    'Ответственность за последствия полностью лежит на администраторе.'
  );

 
  if (!confirm('Вы уверены, что хотите удалить этого пользователя? Действие необратимо.')) {
    return;
  }
  ...
  try {
    ...
    console.error(err);
    showError('Ошибка при удалении пользователя: ' + (err.message || 'Неизвестная ошибка'));
  }
};
